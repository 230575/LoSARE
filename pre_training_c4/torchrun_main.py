...
import sys
sys.path.append("..")
from optimizer_torch import LoSAREAdamW, FiraAdamW, GaLoreAdamW, GaLoreRMSProp, FiraRMSProp  
...
def parse_args(args):
    ...
    parser.add_argument("--adaptive_update", default=False, action="store_true")
    parser.add_argument("--adapt_lr", type=float, default=0.1)
    parser.add_argument("--dynamic_alpha", type=float, default=1.5)
...
def main(args):
    ...
    if global_rank == 0:
        wandb.init(project="pre-training-c4")
    ...
    if any(word in args.optimizer.lower() for word in ['losare', 'fira', 'galore']):
        ...
        for module_name, module in model.named_modules():
            ...
            gradient_projection_params.append(module.weight)
        id_gradient_projection_params = [id(p) for p in gradient_projection_params]
        # make parameters without "rank" to another group
        regular_params = [p for p in model.parameters() if id(p) not in id_gradient_projection_params]
        param_groups = [
            {'params': regular_params}, 
            {'params': gradient_projection_params, 
             'rank': args.rank, 
             'update_proj_gap': args.update_proj_gap, 
             'alpha': args.alpha, 
             'proj_type': args.proj_type,
             'adaptive_update': args.adaptive_update,    # newly added
             'adapt_lr': args.adapt_lr,                  # newly added
             'dynamic_alpha': args.dynamic_alpha         # newly added
            }
        ]
    ...
    if any(word in args.optimizer.lower() for word in ['losare', 'fira', 'galore']):
        logger.info(f"Total params with gradient projection enabled: {sum(p.numel() for p in gradient_projection_params) / 1_000_000:.2f}M")
    logger.info(f"Saving model to {args.save_dir} every {args.save_every} update steps")
    
    layer_wise_flag = False
    if args.optimizer.lower() == "adam":
        optimizer = torch.optim.Adam(trainable_params, lr=args.lr, weight_decay=args.weight_decay)
    elif args.optimizer.lower() == "losare_adamw":
        optimizer = LoSAREAdamW(param_groups, lr=args.lr, weight_decay=args.weight_decay)
    ...
    elif args.optimizer.lower() == "fira_rmsprop":
        optimizer = FiraRMSProp(param_groups, lr=args.lr, weight_decay=args.weight_decay)
